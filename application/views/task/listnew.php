...
<ul class="nav nav-pills" style="margin-bottom: 20px;">
    <li<?php if ($this->input->get('type_id') <= 0) : ?> class="active"<?php endif; ?>><a
                href="<?php echo site_url('task'); ?>">Все</a></li>
    <?php
    foreach ($types as $item) {
        $active = $this->input->get('type_id') == $item->id ? ' class="active"' : '';
        echo '<li' . $active . '><a href="' . site_url('task/?type_id=' . $item->id) . '">' . $item->name . '</a></li>';
    }
    ?>
</ul>
...
<div class="table-responsive">
    <table class="table table-striped table-bordered table-hover">
        ...
        <tr>
            ...
            <th><?php echo $this->lang->line('action_cost'); ?></th>
            ...
            <?php foreach ($results as $item) : ?>
                <tr<?php if ($item->total_cost < $item->action_cost) {
                    ...
                    <td>
						...
						<?php if($item->type_id == TASK_COMMENT && $item->action_done > 10) : ?>
						<?php echo anchor('task/done_recalc/'.$item->id, '<i class="fa fa-refresh"></i>', 'class="btn btn-success btn-circle" title="Пересчитать кол-во выполнений" onclick="YT_DoneRecalc('.$item->id.'); return false;"'); ?> 
						<?php endif; ?>
                        ...
                        <?php if ($item->type_id == TASK_VIEW && $item->action_done > 1) : ?>
                            <?php echo anchor('task/done_recalc/' . $item->id, '<i class="fa fa-refresh"></i>', 'class="btn btn-success btn-circle" title="Пересчитать кол-во выполнений" onclick="YT_DoneRecalc(' . $item->id . '); return false;"'); ?>
                        <?php endif; ?>

                        <a href="/task/remove/<?php echo $item->id; ?>" class="btn btn-danger btn-circle"
                           onclick="return YT_ConfirmDelete();" title="<?php echo $this->lang->line('remove'); ?>"><i
                                    class="glyphicon glyphicon-remove"></i></a>
                    </td>
                </tr>
                <?php if (!empty($item->description)) { ?>
                    <tr>
                        <td colspan="7" style="color: red;"><?php echo $item->description; ?></td>
                    </tr>
                <?php } ?>
            <?php endforeach; ?>
        <?php else : ?>
            <tr>
                <td colspan="7" style="text-align: center;"><?php echo $this->lang->line('no_task'); ?></td>
            </tr>
        <?php endif; ?>
        </tbody>
    </table>
